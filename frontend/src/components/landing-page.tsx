  ...
  return (
    <div className="relative min-h-screen flex bg-black text-slate-50 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col">
        <main className="flex-1 overflow-y-auto px-4 md:px-8 pb-16 space-y-16 bg-black/80">
          <FloatingActionMenu
            options={[
              {
                label: "Logout",
  ...
  const topLocations = [
    {
      ...
      volume: "18.4% of traffic",
      risk: "Elevated",
      tone: "warning" as const,
      detail: "High-value international corridor with increased monitoring.",
    },
    {
      region: "Mumbai → Dubai",
      volume: "9.7% of traffic",
      risk: "High",
      tone: "danger" as const,
      detail: "Spike in card-not-present attempts from new devices.",
    },
    {
      region: "Domestic · India",
      volume: "54.2% of traffic",
      risk: "Normal",
      tone: "success" as const,
      detail: "Stable geo profile with low fraud rate.",
    },
  ];

  return (
    <section id="location" className="space-y-10">
      <div className="container max-w-[1220px] w-full px-6 md:px-10 mx-auto -mt-6 flex justify-center">
        <div className="w-full max-w-3xl space-y-4">
          <SectionTitle
            icon={Globe2}
            label="Location"
            description="Top transaction corridors and their current risk posture."
          />
          <div className="space-y-5">
            <div className="flex items-center justify-between text-xs text-slate-400">
              <span>Geo distribution · Last 24h</span>
              <span>Interactive globe · Drag to explore</span>
            </div>
            <div className="w-full max-w-md mx-auto">
              <GlobeAnalytics className="w-full" />
            </div>
            <div className="space-y-3 text-xs">
              {topLocations.map((loc) => (
                <div
                  key={loc.region}
                  className="rounded-2xl bg-slate-950/80 border border-slate-700/80 px-3 py-3 flex flex-col gap-1"
                >
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex flex-col gap-0.5">
                      ...
                      <span className="text-slate-400">{loc.volume}</span>
                    </div>
                    <Tag tone={loc.tone}>
                      {loc.risk} geo risk
                    </Tag>
                  </div>
                  <p className="text-[11px] text-slate-400">{loc.detail}</p>
                </div>
              ))}
            </div>
          </div>
